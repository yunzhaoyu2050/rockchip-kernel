...
static int power_en_probe(struct platform_device *pdev)
{
	...
	data->gpio_5ven_2.gpio_num = of_get_named_gpio_flags(np, "5ven_pin_2", 0, NULL);
        if (!gpio_is_valid(data->gpio_5ven_2.gpio_num))
                data->gpio_5ven_2.gpio_num = -1;
...
//      	        return ret;
	...
	if(data->gpio_5ven_2.gpio_num != -1){
               ret = gpio_request(data->gpio_5ven_2.gpio_num, "gpio_5ven");
               if (ret < 0){
                       printk("data->gpio_5ven request error\n");
//                     return ret;
               }else{
			   gpio_direction_output(data->gpio_5ven_2.gpio_num, 1);
			   gpio_set_value(data->gpio_5ven_2.gpio_num, 1);
               }
       }
...
static int power_en_remove(struct platform_device *pdev)
{
	...
	if(data->gpio_5ven_2.gpio_num != -1){
               gpio_direction_output(data->gpio_5ven_2.gpio_num, 0);
               gpio_free(data->gpio_5ven_2.gpio_num);
        }
	if(data->hub_rst.gpio_num != -1){
		gpio_direction_output(data->hub_rst.gpio_num, 0);
